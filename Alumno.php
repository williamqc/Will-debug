...
function findAll() {
	 ...
	 */
	$fields = array('id_alumno','nombre','edad');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `alumno` $where"); 
	...
	$query_recordset = "SELECT id_alumno,nombre,edad FROM `alumno` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `alumno` (id_alumno,nombre,edad) VALUES (%s,%s,%s)" ,			GetSQLValueString($_REQUEST["id_alumno"], "int"), # 
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `alumno` WHERE id_alumno = %s",
		GetSQLValueString($_REQUEST["id_alumno"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `alumno` SET nombre = %s,edad = %s WHERE id_alumno = %s", 
			GetSQLValueString($_REQUEST["nombre"], "text"), 
			GetSQLValueString($_REQUEST["edad"], "int"), 
			GetSQLValueString($row_recordset["id_alumno"], "int")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"id_alumno" => $row_recordset["id_alumno"], 
						"nombre" => $_REQUEST["nombre"], #
						"edad" => $_REQUEST["edad"]#
					)
				), 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `alumno` WHERE id_alumno = %s", 
			GetSQLValueString($row_recordset["id_alumno"], "int")
		);
